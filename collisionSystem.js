...
var speed = 10; // change the speed of balls

var fps = 60,  // use a typical monitor refresh rate
...
function FastCollisionSystem() {
  ...
  this.draw = function() {
    ...
    var wait = gameToSystemTime(ev.time - realTime);
    this.lastwait = wait; // Allow the FPS counter to report the last wait time
  ...
  this.loop = function() {
    while (true) {
      ...
      for(var i = 0, length = this.circles.length; i < length; i++) {
        this.circles[i].move(ev.time - this.currentTime, this.canvas.width, this.canvas.height);
      }
...
FastCollisionSystem.prototype = new CollisionSystem();
